...
/**
 * Tela de acompanhamento do transporte de pets
 * 
 * Esta tela permite que o usuário acompanhe o status do transporte de pets em tempo real,
 * visualize a localização do parceiro e receba atualizações.
 * 
 * @returns {React.ReactElement} Componente de tela de acompanhamento do transporte de pets
 */
const PetTransportTrackingScreen = () => {
  ...
  const [showPawPrints, setShowPawPrints] = useState(true);
  ...
  useEffect(() => {
    const statusTimeline = [
      ...
      { status: TRANSPORT_STATUS.DELIVERED, delay: 15000 },
    ];
      ...
      const timeoutId = setTimeout(() => {
        ...
        if (item.status === TRANSPORT_STATUS.PARTNER_FOUND) {
          ...
          setPartner({
            ...
            petTransportVerified: true,
          });
          // Mostrar efeito de pegadas e vibrar
          setShowPawPrints(true);
          Vibration.vibrate(100);
          setTimeout(() => setShowPawPrints(false), 1500);
        } else if (item.status === TRANSPORT_STATUS.PICKUP) {
          setEstimatedTime(10);
        } else if (item.status === TRANSPORT_STATUS.IN_TRANSIT) {
          setEstimatedTime(8);
          setShowPawPrints(false);
        } else if (item.status === TRANSPORT_STATUS.DELIVERED) {
          // Mostrar efeito de pegadas e vibrar
          setShowPawPrints(true);
      ...
      case TRANSPORT_STATUS.SEARCHING:
        return (
          <View style={styles.statusContent}>
            ...
            >
              <LottieView
                source={require('../../../assets/animations/paw-searching.json')}
                autoPlay
                loop
            ...
            <Text style={styles.statusDescription}>
              Estamos encontrando o melhor parceiro para transportar seu pet
            </Text>
          </View>
        );
              ...
              <View style={styles.partnerContainer}>
                ...
                <View style={styles.statusTextContainer}>
                  <Text style={styles.statusTitle}>
                    ...
                    {transportStatus === TRANSPORT_STATUS.PICKUP && 'Buscando seu pet'}
                    {transportStatus === TRANSPORT_STATUS.IN_TRANSIT && 'Transporte em andamento'}
                    {transportStatus === TRANSPORT_STATUS.DELIVERED && 'Transporte concluído!'}
                  </Text>
                  <Text style={styles.statusDescription}>
                    {transportStatus === TRANSPORT_STATUS.PARTNER_FOUND && 'Seu parceiro está indo até o local de coleta'}
                    {transportStatus === TRANSPORT_STATUS.PICKUP && 'Seu parceiro está buscando seu pet'}
                    {transportStatus === TRANSPORT_STATUS.IN_TRANSIT && 'Seu pet está a caminho do destino'}
                    {transportStatus === TRANSPORT_STATUS.DELIVERED && 'Seu pet chegou ao destino com segurança!'}
                  </Text>
                </View>
                
                {transportStatus !== TRANSPORT_STATUS.DELIVERED && (
                ...
                {transportStatus === TRANSPORT_STATUS.DELIVERED && (
                  <TouchableOpacity
                    ...
                    onPress={() => navigation.navigate('PetTransportRating', { partner })}
                  >
...
export default PetTransportTrackingScreen;
